...
export default function BusinessDropdownMenu({ businessId }: { businessId: string }): React.JSX.Element {
  ...
  return (
    <>
      ...
      {anchorEl ? (
        ...
        >
          <MenuItem
            ...
            component={Link}
            href={paths.businesses.details(businessId)}
          >
          ...
          <MenuItem
            sx={{ display: 'flex', flexDirection: 'row', gap: 1, justifyContent: 'flex-start', alignItems: 'center' }}
            component={Link}
            href={paths.businesses.edit(businessId)}
          >
      ...
      {isOpen ? <ConfirmDeleteBusinessModal businessId={businessId} open={isOpen} toggleModal={toggleModal} /> : null}
    </>
  );
}
